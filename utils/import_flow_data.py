...
latest_values AS (
    ...
    FROM {ea_values_at_run_source_sql(
        schema=DB_SCHEMA,
        dataset_ids_sql="import_mappings",
    )} values_table
),
country_monthly AS (
    SELECT
...
def fetch_ea_import_flow_raw_data(
    *,
    ...
    if ea_as_of_run_id is not None:
        return fetch_ea_import_flow_raw_data_at_run(
            ea_as_of_run_id,
...
def fetch_ea_import_flow_raw_data_at_run(
    ea_as_of_run_id: int,
    *,
    start_date: str | None = None,
    end_date: str | None = None,
...
fetch_ea_import_flow_raw_data_for_upload = fetch_ea_import_flow_raw_data_at_run
