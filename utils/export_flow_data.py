...
export_mappings AS (
    SELECT
        ...
        unit,
        frequency
    FROM {DB_SCHEMA}.fundamentals_ea_lng_balance_datasets
...
EA_PARAMETERIZED_EXPORT_FLOW_QUERY = f"""
WITH
{COUNTRY_MAPPING_CTE},
export_mappings AS (
    SELECT
...
EA_EXPORT_FLOW_METADATA_QUERY = f"""
SELECT MAX(upload_timestamp_utc) AS upload_timestamp_utc
FROM {DB_SCHEMA}.ea_values
"""
...
def fetch_all_export_flow_raw_data() -> dict[str, pd.DataFrame]:
    with engine.connect() as connection:
        ...
        ea_df = pd.read_sql_query(EA_EXPORT_FLOW_QUERY, connection)
...
def fetch_ea_export_flow_raw_data_for_upload(upload_timestamp_utc: str) -> pd.DataFrame:
    with engine.connect() as connection:
        ea_df = pd.read_sql_query(
            text(EA_PARAMETERIZED_EXPORT_FLOW_QUERY),
            connection,
...
def fetch_ea_export_flow_metadata() -> dict[str, str | None]:
    with engine.connect() as connection:
        metadata_df = pd.read_sql_query(EA_EXPORT_FLOW_METADATA_QUERY, connection)
...
def fetch_ea_upload_options() -> list[str]:
    with engine.connect() as connection:
        options_df = pd.read_sql_query(EA_UPLOAD_OPTIONS_QUERY, connection)
