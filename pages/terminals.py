...
def get_all_countries(new_capacity_only=False, scenario='base_view'):
    """
    Get list of all countries with LNG output data.

    Args:
        new_capacity_only: If True, only return countries with new capacity from current month onwards
        scenario: Scenario name for adjustments ('base_view', 'best_view', etc.)
    ...
    if new_capacity_only:
        # Get countries with new capacity
        new_trains_df = get_new_capacity_filters(scenario=scenario)
        if new_trains_df.empty:
            return []
        countries = sorted(new_trains_df['country_name'].unique().tolist())
        return countries
    else:
        # Get all countries
        query = f"""
...
def create_volume_area_chart(selected_unit='mtpa', breakdown='country', new_capacity_only=False, selected_countries=None, scenario='base_view', start_year=2025, end_year=2040):
    """
    Create stacked area chart showing cumulative monthly output.

    Args:
        selected_unit: 'mtpa' or 'mcmd'
        breakdown: 'country', 'project', or 'train'
        new_capacity_only: If True, show only new capacity from current month onwards
        selected_countries: List of countries to filter by (None = all countries)
        scenario: Scenario name for adjustments ('base_view', 'best_view', etc.)
        start_year: Starting year for data (default: 2025)
        end_year: Ending year for data (default: 2040)
    ...
    """
    # Fetch data with specified breakdown, scenario, and year range
    df = fetch_volume_data(start_year=start_year, end_year=end_year, breakdown=breakdown, new_capacity_only=new_capacity_only, selected_countries=selected_countries, scenario=scenario)
    ...
    if selected_unit == 'mcmd':
        df['total_output'] = convert_to_mcmd(df['total_output'])

    # Create date column for x-axis
    df['date'] = pd.to_datetime(df[['year', 'month']].assign(day=1))

    # Pivot data for stacking (group_name as columns, dates as index)
    pivot_df = df.pivot(index='date', columns='group_name', values='total_output').fillna(0)

    # Filter out months with no data (where total output across all groups is zero)
    pivot_df = pivot_df[pivot_df.sum(axis=1) > 0]

    # Sort columns by total output (descending) for better visual stacking
    column_totals = pivot_df.sum().sort_values(ascending=False)
    pivot_df = pivot_df[column_totals.index]

    # Create color mapping based on breakdown level
    if breakdown == 'country':
        color_map = {group: PRIMARY_COLORS.get(group, '#666666') for group in pivot_df.columns}
    elif breakdown == 'project' or breakdown == 'train':
        # Optimized color palette with maximum visual distinction
        # Colors carefully selected to maximize perceptual difference
        diverse_colors = [
            ...
            '#dc143c', '#00bfff', '#adff2f', '#ff69b4', '#1e90ff',
            '#ffff54', '#dda0dd', '#87ceeb', '#ff6347', '#48d1cc'
        ]

        # Assign colors sequentially to groups (sorted by total output)
        color_map = {}
        for idx, group in enumerate(pivot_df.columns):
            color_map[group] = diverse_colors[idx % len(diverse_colors)]
    else:
        color_map = {group: '#666666' for group in pivot_df.columns}

    # Create figure
    fig = go.Figure()

    # Add trace for each group (stacked area)
    for group_name in pivot_df.columns:
        color = color_map.get(group_name, '#666666')
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=f'<b>{group_name}</b><br>Date: %{{x|%b %Y}}<br>Output: %{{y:.1f}} {selected_unit.upper()}<extra></extra>',
            stackgroup='one'  # This creates the stacking effect
        ))

    # Update layout
    unit_label = 'MTPA' if selected_unit == 'mtpa' else 'Mcm/d'
    breakdown_label = breakdown.capitalize()

    # Determine date range from actual data
    if not pivot_df.empty:
        start_date = pivot_df.index.min()
        end_date = pivot_df.index.max()
        title_years = f"{start_date.year}-{end_date.year}"
    else:
        start_date = '2025-01-01'
        end_date = '2028-12-31'
        title_years = '2025-2028'

    fig.update_layout(
        title={
            'text': f'Cumulative Monthly LNG Output by {breakdown_label} ({unit_label}) | {title_years}',
...
layout = html.Div([
    ...
    # Volume chart controls - checkbox and breakdown inline
    html.Div([
        html.Div([
            dcc.Checklist(
                id='new-capacity-checkbox',
                options=[{'label': ' New capacity', 'value': 'new_only'}],
                value=[],
                style={'fontSize': '14px', 'fontWeight': 'bold', 'marginRight': '30px'}
            ),
            html.Label("Volume:", style={'fontWeight': 'bold', 'marginRight': '10px'}),
            dcc.Dropdown(
            ...
            html.Label("Countries:", style={'fontWeight': 'bold', 'marginRight': '10px'}),
            dcc.Dropdown(
                id='country-filter-dropdown',
                options=[],  # Will be populated by callback
                value=[],    # Will be set to all countries by callback
                multi=True,
                placeholder='Select countries...',
                className='inline-dropdown',
                style={'width': '800px'}
            ),
            # Select All / Clear All buttons with aesthetic styling
            html.Div([
                html.Button(
                    '✓ All',
                    id='select-all-countries-btn',
                    n_clicks=0,
                    style={
                        'padding': '6px 12px',
            ...
            ], style={'display': 'inline-flex', 'marginLeft': '8px', 'marginRight': '20px'}),
            html.Button(
                'Export to Excel',
    ...
    # Year range filter for volume chart
    html.Div([
        html.Div([
            html.Label("Year Range (Volume Chart):", style={'fontWeight': 'bold', 'marginRight': '10px'}),
            dcc.RangeSlider(
                id='volume-year-range-slider',
                min=2000,
                max=2055,
                step=1,
                value=[2025, 2040],
...
# Callback to populate country filter dropdown on page load and when new capacity checkbox changes
@callback(
    Output('country-filter-dropdown', 'options'),
    Output('country-filter-dropdown', 'value'),
    Input('new-capacity-checkbox', 'value'),
    Input('global-refresh-button', 'n_clicks')
)
def populate_country_filter(new_capacity_checkbox, n_clicks):
    """Populate country filter dropdown based on new capacity filter."""
    # Check if "new only" is selected
    new_capacity_only = 'new_only' in (new_capacity_checkbox or [])

    countries = get_all_countries(new_capacity_only=new_capacity_only)
    options = [{'label': country, 'value': country} for country in countries]
    # By default, select all available countries
    return options, countries


# Callback to handle Select All and Clear All buttons for countries
@callback(
    Output('country-filter-dropdown', 'value', allow_duplicate=True),
    Input('select-all-countries-btn', 'n_clicks'),
    Input('clear-all-countries-btn', 'n_clicks'),
    State('country-filter-dropdown', 'options'),
    State('country-filter-dropdown', 'value'),
    prevent_initial_call=True
)
def handle_country_selection_buttons(select_all_clicks, clear_all_clicks, options, current_value):
    """Handle Select All and Clear All button clicks for country filter."""
    ...
    if button_id == 'select-all-countries-btn':
        # Select all countries
        all_countries = [opt['value'] for opt in options]
        return all_countries
    elif button_id == 'clear-all-countries-btn':
...
# Callback to update volume chart (depends on all filters)
@callback(
    Output('volume-area-chart', 'figure'),
    Input('scenario-selector-dropdown', 'value'),
    Input('unit-dropdown', 'value'),
    Input('breakdown-dropdown', 'value'),
    Input('new-capacity-checkbox', 'value'),
    Input('country-filter-dropdown', 'value'),
    Input('volume-year-range-slider', 'value'),
    Input('global-refresh-button', 'n_clicks')
)
def update_volume_chart(scenario, selected_unit, breakdown, new_capacity_checkbox, selected_countries, volume_year_range, n_clicks):
    """Update volume chart based on scenario, unit, breakdown, checkbox, countries, year range, and refresh button."""
    # Check if "new only" is selected
    new_capacity_only = 'new_only' in (new_capacity_checkbox or [])

    # Extract start and end years from range
    start_year, end_year = volume_year_range if volume_year_range else [2025, 2040]

    return create_volume_area_chart(selected_unit, breakdown, new_capacity_only, selected_countries, scenario=scenario, start_year=start_year, end_year=end_year)


# Callback to export volume data to Excel
@callback(
...
def export_to_excel(n_clicks, selected_unit, breakdown, new_capacity_checkbox, selected_countries):
    """Export volume chart data to Excel file."""
    if n_clicks == 0:
        return None
    ...
    # Fetch the data
    df = fetch_volume_data(breakdown=breakdown, new_capacity_only=new_capacity_only, selected_countries=selected_countries)
    ...
    # Create date column for better readability
    df['date'] = pd.to_datetime(df[['year', 'month']].assign(day=1))

    # Pivot data for Excel export (dates as rows, groups as columns)
    pivot_df = df.pivot(index='date', columns='group_name', values='total_output').fillna(0)
    ...
    # Rename date column
    pivot_df = pivot_df.rename(columns={'date': 'Year-Month'})

    # Create Excel file in memory
    output = BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        pivot_df.to_excel(writer, sheet_name='Volume Data', index=False)
        ...
        for idx, col in enumerate(pivot_df.columns):
            ...
            max_length = max(
                pivot_df[col].astype(str).apply(len).max(),
                len(str(col))
            ) + 2

            # Convert column index to Excel column letter (handles A-Z, AA-ZZ, etc.)
            col_letter = ''
            temp_idx = idx + 1
            while temp_idx > 0:
                temp_idx -= 1
    ...
    filename = f'LNG_Volume_{breakdown_label}_{unit_label}{new_cap_label}_{timestamp}.xlsx'
